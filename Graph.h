...
//用于大家查数据的函数
    ...
    //增、删、改
    Q_INVOKABLE bool expand_point(string& new_point_name, int addr_x,int addr_y,string& former_point_name, float length, string& road_name,string new_point_intro = "");  //扩充景点并形成新的路
    Q_INVOKABLE bool expand_point(string& new_point_name, int addr_x,int addr_y,string new_point_intro = "");   //扩充单个景点（用于初始化）
    Q_INVOKABLE bool expand_road(string& road_name,string& point1, string& point2, float length);       //扩充路径
    Q_INVOKABLE bool expand_road(string& road_name,int point1_key, int point2_key, float length);       //扩充路径


    Q_INVOKABLE bool update_point_name(int point_key,string& new_point_name);          //更改点的名称
    Q_INVOKABLE bool update_point_intro(int point_key,string& new_point_intro);        //更改点的介绍
    Q_INVOKABLE bool update_point_add(int point_key,int new_add_x,int new_add_y);      //更改点的坐标
    Q_INVOKABLE bool update_road_name(int road_key,string& road_name);                 //更改路的名称
